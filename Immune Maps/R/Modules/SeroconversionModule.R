...
SeroconversionServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$VolcanoPlotTitle <- renderUI({
      ...
      title <- ifelse(input$VolcanoDatasetRefresh,paste0('Effect of ',groupVariableLabel,' on all ',rv$Platform,' '),'Please start by setting dataset options below')
    ...
    AnalyteDataset <- reactive({ 
      ...
      dataframe <- dataframe %>%
        mutate(highlightGroup = case_when(RecordID %in% groupA ~ "A", RecordID %in% groupB ~ "B")) %>% 
        select(-c(AgeGroup,Measurement,Kit_Barcode,ExperimentID, Cell_population)) %>%
