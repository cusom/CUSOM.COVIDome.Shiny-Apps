...
KaryotypeServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$VolcanoPlotTitle <- renderUI({
      ...
      title <- ifelse(input$VolcanoDatasetRefresh,paste0('Effect of ',analyteLabel,' on all genes in Whole Blood'),'Please start by setting dataset options below')
    ...
    output$AnalyteBoxPlotPlotTitle <- renderUI({
      ...
      if(nrow(p) > 0) {
        ...
        HTML(
          paste0(
            '<h3>Effect of ',analyteLabel,' on ',input$Analyte,' in Whole Blood
