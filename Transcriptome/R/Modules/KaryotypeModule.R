...
KaryotypeServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(c(input$Platform),{
      ...
      rv$analytePlaceholder <- "Please select a Gene below"
      AnalyteLabel <- "Highlight Gene (optional)"
      rv$analyteLabel <- "Gene"
      analyteChoices <- genes
    ...
    output$VolcanoPlotTitle <- renderUI({
      ...
      title <- ifelse(input$VolcanoDatasetRefresh,paste0('Effect of ',groupVariableLabel,' on all genes in Whole Blood'),'Please start by setting dataset options below')
    ...
    GetAnalyteData <- function(platform,sex,ageGroup,analyte,groupA,groupB) {
      ...
      dataframe <- dataframe %>%
        ...
        rename( `Seroconverion Group` = SeroconversionGroup,`Highlight Group` = highlightGroup) %>%
        select(RecordID,Analyte,Platform,Sex,GroupVariable,`Seroconverion Group`,`Highlight Group`,MeasuredValue) %>%
    ...
    output$AnalyteBoxPlotPlotTitle <- renderUI({
      ...
      if(nrow(p) > 0) {
        ...
        HTML(
          paste0(
            '<h3>Effect of ',groupVariableLabel,' on ',input$Analyte,' in Whole Blood
