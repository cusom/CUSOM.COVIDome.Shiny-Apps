...
SeroconversionServer <- function(id) {
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(c(input$Platform),{
      ...
      rv$analytePlaceholder <- "Choose a cytokine"
      AnalyteLabel <- "Highlight Cytokine (optional)"
    ...
    output$VolcanoPlotTitle <- renderUI({
      ...
      title <- ifelse(input$VolcanoDatasetRefresh,paste0('Effect of ',groupVariableLabel,' on all ',ifelse(grepl('MSD',input$Platform),'cytokines','aptamers')),'Please start by setting dataset options below')
    ...
    GetAnalyteData <- function(platform,sex,ageGroup,analyte,groupA,groupB) {
      ...
      dataframe <- dataframe %>%
        ...
        rename(`Specimen Type` = Specimen_type, `COVID-19 Status` = Status,`Highlight Group` = highlightGroup) %>%
        select(RecordID,`Specimen Type`,Analyte,Platform,Sex,`COVID-19 Status`,GroupVariable,`Highlight Group`,MeasuredValue) %>%
